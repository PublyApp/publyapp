...
- **Client route loaders are sanctioned by a guard, not by prose (#1552/#1560):** the only permitted client `loader` warms the SAME TanStack Query options the route's own components pass to `useQuery` (query-key-set inclusion, never equality). `apps/front/src/lib/route-loader-query-key-guard.test.ts` enforces it statically over the real `src/routes/` modules and fails loud (naming the route) on any loader it cannot resolve — a dynamic loader is not a silent pass. It runs in CI via `ci.yml` → job `verification` → step "Test front (non-vitest report-all)". See [`docs/guides/front/conventions.md`](docs/guides/front/conventions.md) §"Client route loaders (#851)".
