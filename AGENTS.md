...
hooks. CI wires hooks explicitly via the ci.yml `verification` job step
"Install Git hooks" (`pnpm run prepare`).
...
**CI runs the API suite since #1462** (`.github/workflows/ci.yml` runs
`just test-api` in its API lane), so `just ci` mirrors it locally rather than
exceeding CI on the backend. The central workflow is now the sole PR validation
workflow; PR B removed the eight predecessor workflows.
...
- **React Doctor HARD gate:** a PR must not leave any React Doctor finding in a file it changes. Run `just react-doctor` before pushing. CI enforces this via `.github/workflows/ci.yml` (`--scope files --blocking warning`). Full guide: [`docs/guides/react-doctor.md`](docs/guides/react-doctor.md).
- **Client route loaders are sanctioned by a guard, not by prose (#1552/#1560):** the only permitted client `loader` warms the SAME TanStack Query options the route's own components pass to `useQuery` (query-key-set inclusion, never equality). `apps/front/src/lib/query/route-loader-query-key-guard.test.ts` enforces it statically over the real `src/routes/` modules and fails loud (naming the route) on any loader it cannot resolve — a dynamic loader is not a silent pass. It runs in CI via `ci.yml` → job `verification` → step "Test front (non-vitest report-all)". See [`docs/guides/front/conventions.md`](docs/guides/front/conventions.md) §"Client route loaders (#851)".
...
**Front bug/regression paired red/green proofs:** a PR opts into this convention by adding or
modifying a proof under `apps/front/tests/proofs/<issue>/` (versionned, committed). The test that
proves a bug is present cannot stay in the normal suite — by construction it FAILS against the
corrected code — so the CI step `Verify paired red proofs` replays only declared proofs with inverted
semantics. The trace names the path and mutation that reproduces the red. This is not a universal
kept-red requirement for repository surfaces without a supported proof runner/location; they use
ordinary regression tests and review evidence. Full rule, legacy API trace form, and rationale in
[`docs/guides/test-conventions.md`](docs/guides/test-conventions.md) §"Paired Red/Green Proofs —
keeping the red test alive". Bespoke-guard admission evidence instead follows the fixture/temporary-
mutation rule in the hard admission section above.
