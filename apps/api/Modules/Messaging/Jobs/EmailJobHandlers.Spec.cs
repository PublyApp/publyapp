...
// barriers and failures. The processor-dispatch specs use the production registry and
// per-job DI scope to prove enqueue -> claim -> handler -> terminal persistence end to end.
public sealed class EmailJobHandlersSpec : IClassFixture<ApiFixture> {
	...
	public async Task ItShouldDispatchPermanentEmailFailureThroughTheRegisteredProcessor() {
		...
		JobQueueProcessor.BatchResult result;
		try {
			result = await CreateProcessor().ProcessBatchAsync(CancellationToken.None);
		} finally {
			sender.FailWith = null;
		}
