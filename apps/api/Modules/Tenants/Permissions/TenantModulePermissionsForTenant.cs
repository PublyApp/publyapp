...
namespace PublyApp.Api.Modules.Tenants.Permissions;
	...
	public TenantModulePermissionsForTenant() {
		...
		ACCESS_DASHBOARD = Permission
			.CreateTenantPermission(
				string.Join(Permission.KeySeparator, new[] { KeyPrefix, "access_dashboard" })
			)
			.SetTranslation(
			...
			.SetTranslation(
				...
				new PermissionTranslation {
					...
					Description = "Accéder au tableau de bord du tenant"
				}
			);

		ACCESS_BILLING = Permission
			.CreateTenantPermission(
				string.Join(Permission.KeySeparator, new[] { KeyPrefix, "access_billing" })
			)
			.SetTranslation(
				SupportedLanguage.English,
				new PermissionTranslation {
					Name = "Access billing",
					Description = "Access tenant billing"
				}
			)
			.SetTranslation(
				SupportedLanguage.French,
				new PermissionTranslation {
					Name = "Accéder à la facturation",
					Description = "Accéder à la facturation du tenant"
				}
			);

		ACCESS_SETTINGS = Permission
			.CreateTenantPermission(
				string.Join(Permission.KeySeparator, new[] { KeyPrefix, "access_settings" })
			)
			.SetTranslation(
				SupportedLanguage.English,
				new PermissionTranslation {
					Name = "Access settings",
					Description = "Access tenant settings"
				}
			)
			.SetTranslation(
				SupportedLanguage.French,
				new PermissionTranslation {
					...
					Description = "Accéder aux paramètres du tenant"
				}
			);

		ACCESS_USERS = Permission
			.CreateTenantPermission(
				string.Join(Permission.KeySeparator, new[] { KeyPrefix, "access_users" })
			)
			.SetTranslation(
				SupportedLanguage.English,
				new PermissionTranslation {
					Name = "Access users",
					Description = "Access tenant users"
				}
			)
			.SetTranslation(
				SupportedLanguage.French,
				new PermissionTranslation {
					...
					Description = "Accéder aux utilisateurs du tenant"
				}
			);
	}
}
