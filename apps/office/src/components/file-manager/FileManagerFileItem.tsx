...
const FileManagerFileItem = ({ file, selected, onSelect, onDelete, sx, ...other }: Props) => {
	...
	const renderIcon = <FileThumbnail /* file={file.type} */ file={file.url} sx={{ width: 36, height: 36 }} />;
	...
	const renderText = (
		<>
			<TextMaxLine
				...
				sx={{ width: 1, mt: 2, mb: 0.5, overflowWrap: 'break-word' }}
			>
				{file.name}
			</TextMaxLine>
	...
	const renderAvatar = (
		...
		>
			{file.shared?.map((person) => {
				return <Avatar key={person.id} alt={person.name} src={person.avatarUrl} />;
			})}
		</AvatarGroup>
	);
		...
		<>
			<Stack
				...
				sx={{
					...
					// 	boxShadow: (theme) => {
					// 		return theme.customShadows.z20;
					// 	},
					// }),
					...sx,
				}}
