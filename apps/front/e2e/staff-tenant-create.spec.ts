...
test.describe(
	'staff create-tenant workspace slug and owners section',
...
test.describe(
	'staff create-tenant shell (rail-only) and two-pane layout',
	{ tag: ['@staff-tenants', '@806'] },
	() => {
		test('the create route is rail-only and lays out the form and preview side by side on desktop', async ({
			...
			const formNameField = page.getByRole('textbox', {
				name: /organization/i,
			});
...
test.describe(
	'staff create-tenant preview counts',
	{ tag: ['@staff-tenants', '@806'] },
	() => {
...
test.describe(
	'staff create-tenant submission',
	{ tag: ['@staff-tenants', '@806'] },
	() => {
...
test.describe(
	'staff create-tenant Organization details optional section',
	{ tag: ['@staff-tenants', '@806'] },
	() => {
...
test.describe(
	'staff create-tenant submit confirmation',
	{ tag: ['@staff-tenants', '@806'] },
	() => {
		test('shows a rich summary confirm dialog before submitting, and Cancel returns to the form untouched', async ({
			...
			await expect(
				dialog.getByTestId('confirm-create-tenant-owners'),
			).toHaveText('1');
...
test.describe(
	'staff create-tenant logo upload',
	{ tag: ['@staff-tenants', '@806'] },
	() => {
		...
		test('an oversized file shows an inline error and does not set the logo', async ({
			...
			await expect(
				page.getByText('Image must be 2 MB or smaller'),
			).toBeVisible();
