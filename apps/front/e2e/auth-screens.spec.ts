...
test.describe('auth screens', { tag: ['@auth', '@806'] }, () => {
	...
	test('/verify-email renders the request form by default', async ({
		page,
	}) => {
	...
	test('/accept-invitation with a garbage token shows the shared invalid-link view', async ({
		...
		await page.goto(
			'/accept-invitation?id=not-a-real-id&token=not-a-real-token',
		);
