...
function StaffTenantUserDetailsPage() {
	...
	// Hoisted so the chained-query gate reads plain locals, not raw query flags.
	...
	return (
		...
		>
			{({ data: tenantData }) => {
				const tenant = toStaffTenantDetails(tenantData);
				if (!tenant) {
					return (
						<AppErrorView
							icon={<IconAlertCircle aria-hidden="true" className="size-7" />}
				...
				return (
					...
					>
						{({ data: detailsData }) => {
							const user = toStaffTenantUserDetails(detailsData);
							if (!user) {
								return (
									<AppErrorView
										icon={
											<IconSearchOff aria-hidden="true" className="size-7" />
										}
										code={t('error-404-code')}
										title={t('tenant-user-not-found-title')}
							...
							const normalizedStatus = getNormalizedTenantUserStatus(
								user.status,
							);
							const canSuspend = normalizedStatus === TENANT_USER_STATUS_ACTIVE;
							const canReactivate =
								normalizedStatus === TENANT_USER_STATUS_SUSPENDED;
							...
							const isAnyActionPending =
								isStatusActionPending || isRemoveActionPending;

							const membershipAction =
								getMembershipActionLabel(normalizedStatus);
							...
							const handleMembershipAction = async (
								...
								try {
									if (action === 'suspend') {
										await suspendTenantUserMutation.mutateAsync({
											tenantId,
											userId,
										});
									} else {
										await reactivateTenantUserMutation.mutateAsync({
											tenantId,
											userId,
										});
									}
							...
							const handleRemoveAction = async () => {
								try {
									await removeTenantUserMutation.mutateAsync({
										tenantId,
										userId,
									});
								} catch (error) {
