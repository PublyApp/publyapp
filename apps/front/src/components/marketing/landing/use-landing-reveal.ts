 ...
 *    reachable through `data-landing-reveal="idle"`, an attribute no server
...
export const useLandingReveal = (
	...
	useEffect(() => {
		...
		const targets = Array.from(
			container.querySelectorAll<HTMLElement>('[data-landing-reveal-target]'),
		);
		...
		for (const target of targets) {
			...
			target.dataset.landingReveal = 'idle';
			pending.add(target);
		}
		...
		const observer = new IntersectionObserver(
			(entries) => {
				for (const entry of entries) {
					...
					target.dataset.landingReveal = 'in';
