 ...
 * (`landing-05.css`), so there is no seam where the field begins under it.
 ...
 *
 * Content is flush with the ruled column's edges (`.publy-l05-ruled-column`)
 * so its vertical mullions read as continuous through the header, the
 * document body and the footer.
 */
export const Landing05Header = () => {
	const { t } = useTranslation('landing-05');

	return (
		<header
			data-testid="landing-05-header"
			className="sticky top-0 z-(--publy-z-shell-topbar) border-b border-(--publy-border) bg-(--publy-background)"
		>
			<div className="publy-l05-ruled-column flex h-(--publy-header-height) items-center gap-4 px-4 sm:px-6">
				<MarketingBrand />
				<nav
				...
				>
					<a
						href="#product-window"
						className="publy-l05-pressable publy-l05-focus-ring publy-type-sky-label rounded-[var(--publy-radius-small-control)] text-(--publy-foreground-secondary) no-underline hover:text-(--publy-foreground)"
					>
				...
				<div className="ml-auto flex items-center gap-2">
					...
					<Link
						...
						className={cn(
							...
							'publy-l05-pressable whitespace-nowrap no-underline',
						)}
					>
						{t('landing-nav-cta')}
					</Link>
