 ...
 */
export const Landing05ACookieBand = ({
	onCustomize,
}: {
	onCustomize: () => void;
}) => {
	...
	return (
		<div
			role="region"
			aria-labelledby="landing-05-a-cookie-band-title"
			data-testid="landing-05-a-cookie-band"
		...
		>
			<div className="mx-auto flex w-full max-w-(--publy-container-reading) flex-col gap-4 border-t border-(--publy-border) bg-(--publy-surface-raised) p-5 px-4 shadow-[var(--publy-shadow-menu)] sm:px-6 md:flex-row md:items-center md:justify-between md:rounded-[var(--publy-radius-control)] md:border-t-0">
				<div className="flex flex-col gap-1">
					<p
						id="landing-05-a-cookie-band-title"
				...
				<div className="flex flex-col gap-2 sm:flex-row sm:items-center">
					<button
						type="button"
						onClick={onCustomize}
						data-testid="landing-05-a-cookie-band-customize"
						className={cn(
					...
					<button
						type="button"
						onClick={rejectAll}
						data-testid="landing-05-a-cookie-band-reject"
						className={cn(
					...
					<button
						type="button"
						onClick={acceptAll}
						data-testid="landing-05-a-cookie-band-accept"
						className={cn(
