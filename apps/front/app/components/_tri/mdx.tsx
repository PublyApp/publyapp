...
function CustomHeading(props: any) {
	...
	return React.createElement(
		...
		{
			id: slug,
			className: clsx('scroll-mt-36 md:scroll-mt-24 inline-flex', props.className),
		},
		[
...
export const H3 = ({ children }: React.HTMLProps<HTMLHeadingElement>) => (
	<CustomHeading className="mb-2 font-semibold normal-case tracking-tight text-gray-900 dark:text-gray-50" level={3}>
		{children}
	</CustomHeading>
);
...
export const Ul = (props: React.HTMLAttributes<HTMLUListElement>) => (
	<ul className="mb-10 ml-[30px] list-['–__'] space-y-1 leading-8 text-gray-600 dark:text-gray-400" {...props} />
);
...
export function CustomLink(props: any) {
	let href = props.href;
	const style = 'text-indigo-600 font-medium hover:text-indigo-500 dark:text-indigo-500 dark:hover:text-indigo-400';
	if (href.startsWith('/')) {
		return (
...
export const ChangelogEntry = ({ version, date, children }: { version: string; date: string; children: any }) => (
	<div className="relative my-20 flex flex-col justify-center gap-x-14 border-b border-gray-200 md:flex-row dark:border-gray-800">
		<div className="mb-4 md:mb-10 md:w-1/3">
			<div className="sticky top-24 flex items-center space-x-2 md:block md:space-x-0 md:space-y-1.5">
				...
				<span className="block whitespace-nowrap text-sm text-gray-600 dark:text-gray-400">{date}</span>
			</div>
		</div>
