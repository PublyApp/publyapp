...
>(({ className, icon, ...props }, forwardedRef) => {
	...
	return (
		<RadioGroupPrimitives.Item
			ref={forwardedRef}
			className={cx('group relative flex size-8 appearance-none items-center justify-center outline-hidden', className)}
			{...props}
		>
			<div
				className={cx(
...
const ThemeSwitch = () => {
	...
	return (
		...
		>
			<RadioGroupItem aria-label="Switch to System Mode" icon={RiComputerLine} value="system" id="system" />

			<RadioGroupItem aria-label="Switch to Light Mode" icon={RiSunLine} value="light" id="light" />

			<RadioGroupItem aria-label="Switch to Dark Mode" icon={RiMoonLine} value="dark" id="dark" />
		</RadioGroup>
	);
};
