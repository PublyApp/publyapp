...
const COMPOSE_FILE = 'apps/front-2-spike/docker-compose.test.yml';
...
const REPO_ROOT = process.cwd().endsWith('/apps/front-2-spike')
	? resolve(process.cwd(), '../..')
	: process.cwd();
...
const getStaffSessionTokenFromCookie = async (page: Page): Promise<string> => {
	return page.evaluate((cookieKey) => {
		const encodedCookieValue = document.cookie
			.split('; ')
			.find((cookiePart) => cookiePart.startsWith(`${cookieKey}=`))
			?.split('=')[1];
