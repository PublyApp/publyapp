...
vi.mock('react-i18next', () => ({
	useTranslation: () => ({
		t: (key: string, options?: Record<string, unknown>) => {
			if (key === 'edit-item') {
				return `Edit ${String(options?.item ?? '')}`;
			}
...
describe('staff tenant profile edit route', () => {
	beforeEach(() => {
		vi.clearAllMocks();
	...
	test('renders the tenant shell, back link, and prefilled edit values', () => {
		renderPage();

		expect(screen.getByTestId('staff-tenant-profile-edit-page')).toBeTruthy();
		expect(screen.getByText('Acme Corporation')).toBeTruthy();
		expect(screen.getByDisplayValue('Approvers')).toBeTruthy();
		expect(screen.getByDisplayValue('Can review approvals')).toBeTruthy();
		expect(
			screen
				.getByRole('link', { name: 'Back to profile' })
				.getAttribute('href'),
		).toBe(
	...
	test('submits name and description, invalidates tenant profile queries, and navigates back to details', async () => {
		...
		fireEvent.change(screen.getByRole('textbox', { name: 'Profile name' }), {
			target: { value: 'Final approvers' },
		});
		fireEvent.change(screen.getByRole('textbox', { name: 'Description' }), {
			target: { value: 'Can approve final decisions' },
		});
		fireEvent.submit(
		...
		await waitFor(() =>
			expect(mutateAsync).toHaveBeenCalledWith({
				tenantId: '11111111-1111-1111-1111-111111111111',
				profileId: '22222222-2222-2222-2222-222222222222',
				name: 'Final approvers',
				description: 'Can approve final decisions',
			}),
		);
		await waitFor(() =>
	...
	test('renders logout redirect for detail-query 401 failures', () => {
		mocks.useStaffTenantProfileDetailsQuery.mockReturnValue(
			buildQueryResult({
