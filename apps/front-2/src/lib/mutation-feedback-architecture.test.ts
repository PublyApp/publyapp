...
describe('front-2 mutation feedback architecture', () => {
	test('production source discovery is non-empty', async () => {
		const files = await getProductionSourceFiles();
		expect(files.length).toBeGreaterThan(0);
	});

	test('Sonner imports stay behind the two front-2 presentation adapters', async () => {
		const files = await getProductionSourceFiles();
	...
	test('front-2 query factories never configure handlers.onToast', async () => {
		const files = await getProductionSourceFiles();
		const offenders = files
