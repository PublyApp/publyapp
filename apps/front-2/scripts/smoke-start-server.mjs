...
// Local mirror of the "Smoke start front-2 server and verify stylesheet tag"
// step in .github/workflows/front-2-ci.yml.
//
...
const fail = async (message) => {
	...
	console.error('--- front-2 server output ---');
...
if (html === null) {
	await fail(`front-2 standalone server failed to return HTML from ${origin}`);
}

if (!stylesheetPattern.test(html)) {
	await fail('front-2 production HTML does not contain a stylesheet link');
}

await stopServer();

console.log('front-2 smoke start: server served HTML linking a CSS asset [OK]');
