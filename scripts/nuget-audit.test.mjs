              ...
              topLevelPackages: [
                {
                  ...
                  vulnerabilities: [
                    {
                      severity: 'High',
                      advisoryurl: 'https://nvd.nist.gov/vuln/detail/CVE-2024-21907',
                    },
                  ],
                },
              ],
...
test('top-level dotnet problems are surfaced as uninspectable', function () {
  var reports = new Map();
  reports.set('src/Broken/Broken.csproj', topProblemsReport([
    { level: 'error', text: 'Unable to read a package reference from the project.' },
  ]));
  var r = evaluateAudit(reports);
  assert.equal(r.exitCode, 1);
  assert.match(r.uninspectable[0], /could not inspect Broken\.csproj/);
...
test('unparseable JSON is an error and exits 1', function () {
  var reports = new Map();
  reports.set('src/Bad/Bad.csproj', { parsed: null, error: 'unparseable JSON output' });
  var r = evaluateAudit(reports);
  assert.equal(r.exitCode, 1);
