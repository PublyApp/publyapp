...
const ENVIRONMENT_GENERATION_MARKER = '# Generated by review-front-2 — do not edit';
...
const isGhMissingReference = (message) => {
	...
	return (
		((lowered.includes('could not find') ||
			...
			(lowered.includes('pullrequest') || lowered.includes('pull request') || lowered.includes('issue'))) ||
...
const runGhJson = (args, options = {}) => {
	...
	if (isGhAuthFailure(message)) {
		throw new Error(`GitHub auth is required to resolve PR/issue references. ${message}`);
	}
...
const choosePort = async (
	...
	{
		host,
		probePortAvailable = isPortAvailableOnHost,
	} = {},
) => {
...
const runPrByNumber = (number) => runGhJson([
	'pr',
	'view',
	String(number),
	'--json',
...
const resolveByNumber = async (number, worktrees) => {
	...
	if (pr) {
		...
		const suggested = path.join(repoRoot, '.worktrees', `${FRONTEND_TOKEN_PREFIX}${number}`);
		...
		console.error(
			`Could not resolve PR #${number} by branch or head SHA.`,
		);
...
const resolveFrontendToken = async (resolved, options = {}) => {
	...
	if (resolved.kind === 'issue' && resolved.worktree) {
		...
		if (resolved.worktree.branch) {
			const issuePr = await resolver(resolved.worktree.branch);
			if (issuePr?.number) {
...
const generateEnv = (worktreePath, frontendToken, port) => {
	...
	const lines = [
		...
		...FRONTEND_ENV_KEYS.map((key) => `${key}=${envValues[key]}`),
	];
...
const ensureDependencies = (worktreePath) => {
	...
	const assertPinnedPath = path.join(worktreePath, 'apps', 'front-2', 'scripts', 'assert-pinned.mjs');
	if (existsSync(assertPinnedPath)) {
		...
		console.error('Skipping assert-pinned check (missing target script): apps/front-2/scripts/assert-pinned.mjs');
	}
	...
	const sharedTsPackage = path.join(worktreePath, 'packages', 'shared-ts', 'package.json');
	...
	if (existsSync(sharedTsPackage)) {
		...
		console.error('Skipping shared-ts postinstall (missing packages/shared-ts/package.json)');
	}
};
...
const parseAddressForFrontend = async (frontendHost, { dnsLookup = lookup } = {}) => {
...
const launchVite = async (worktreePath, bindHost, publicHost, port) => {
...
const main = async () => {
	...
	const bindHost = await parseAddressForFrontend(frontendHost);
	...
	if (port !== preferredPort) {
		console.log(`Port ${String(port)} was selected because ${String(preferredPort)} is unavailable; this preserves cookie safety while keeping the PR session accessible.`);
	}
