 ...
 *
 * @param {Map<string, { parsed: object | null, error?: string, exitCode?: number }>} reportsByProject
 *   Map of csproj path -> parsed JSON (or null with an error string).
 *   exitCode is the dotnet process exit code (0 = success).
...
export const evaluateAudit = (reportsByProject) => {
	...
	for (const [proj, report] of reportsByProject) {
		...
		// Non-zero dotnet exit means the scan may be unreliable even if JSON is
		// present -- the gate must not silently pass such a project.
		if (report.exitCode && report.exitCode !== 0) {
			uninspectable.push(
				`${proj}: could not inspect ${name}: dotnet list exited with code ${report.exitCode}`,
			);
			continue;
		}
		...
		if (!frameworks || frameworks.length === 0) {
			const problems =
				report.parsed?.problems ?? report.parsed?.projects?.[0]?.problems ?? [];
			const reason =
				problems.length > 0
					? problems
