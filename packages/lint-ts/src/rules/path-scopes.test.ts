...
describe('path-scopes helper', () => {
	...
	it('recognizes absolute front source paths', () => {
	...
	it('does not scope old-front paths anymore', () => {
		const oldFront = '/repo/apps/old-front/src/routes/overview/page.tsx';
		const oldFrontComponent = '/repo/apps/old-front/src/components/page.tsx';

		assert.strictEqual(isFrontSourceFile(oldFront), false);
		assert.strictEqual(isUnderFrontSource(oldFront), false);
		assert.strictEqual(getSourceRelativePath(oldFront), '');
		assert.strictEqual(isFrontComponentTsxFile(oldFrontComponent), false);
		assert.strictEqual(
			isFrontSourceFile('apps/old-front/src/routes/x.ts'),
			false,
		);
		assert.strictEqual(
			isUnderFrontSource('apps/old-front/src/routes/x.ts'),
			false,
		);
	});
});
