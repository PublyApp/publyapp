...
// #1968 — the loud-timeout wrapper used to live in this file, but Vitest
// re-collects the suites of any imported `.test.ts` file, so
// `func-style-loud-timeout.test.ts`'s value-import of `withLoudTimeout`
// caused the ENTIRE config suite (61 heavy legs that plant files into
// the real worktree) to execute in two parallel workers, with their
// `afterAll(removePlanted)` racing each other — a false-red source on the
// very legs the PR hardens, plus doubled wall-clock exposure.
//
// The wrapper now lives in a plain source module at
// `packages/lint-ts/src/lib/with-loud-timeout.ts`. Both this file and
// `func-style-loud-timeout.test.ts` import it from there; vitest only
// collects `*.test.ts` files, so the source module is loaded but never
// re-collected. The collection count drops from 2 files to 1.
import { withLoudTimeout } from '../lib/with-loud-timeout.ts';
	...
	describe('suppression inventory leg — inline disable cannot bypass the production-tree guard', () => {
		...
		it('the full workspace scan covers packages/scripts-ts/ (not just apps/front/)', async () => {
			...
			const { entries: foundEntries } = await withLoudTimeout(
				'scripts-ts-scan',
				30_000,
				() => scanFuncStyleSuppressions(scriptsRoot),
			);
			...
			async () => {
				...
				if (undocumented.length > 0) {
					const names = undocumented
						...
						.join('\n  ');
					assert.fail(
						`undocumented func-style suppressions found:\n  ${names}`,
					);
				}
			...
			async () => {
				...
				try {
					...
					const plantedDir = join(
						WORKSPACE_ROOT,
						'apps/front/proof-1909-not-ignored',
					);
					const { entries: foundEntries, elapsedMs } = await withLoudTimeout(
						'leg 2',
						30_000,
