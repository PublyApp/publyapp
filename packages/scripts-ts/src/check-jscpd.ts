...
/**
 * Known generated files explicitly excluded from BOTH gated surfaces.
 * Unlike pattern-based exclusions, each entry here documents WHY it is
 * generated and where it comes from, so a future `.g.cs` file that is
 * actually hand-maintained is NOT silently hidden (issue #1897).
 *
 *   - apps/api/Localization/ResponseKeys.g.cs: auto-generated by
 *     PublyApp.Scripts (generate-translation-keys) from
 *     packages/shared-ts/src/lib/i18n/json/response-message.en.json,
 *     tracked because the API needs the keys available before the
 *     generator runs (see PublyApp.Api.csproj GenerateTranslationKeys
 *     target). It is regenerated on every non-Test build.
 */
...
/**
 * Test-infrastructure directories. These are compiled ONLY into the test
 * project (PublyApp.Api.Tests.csproj) and never ship, so their duplication
...
/**
 * Spec/test file patterns — excluded from BOTH gated surfaces (pairs AND
 * self-duplication). C# `*.Spec.cs` is this repo's standard co-located
 * test suffix (AGENTS.md); `*.Tests.cs` covers the test-suite variant.
 *
 * Before #1821-r2 the C# suffixes were missing, so C# spec duplication
 * tripped the production ratchet and made up 80.6% of the AUTO metric.
 * Before #1895-r1, test-infrastructure files without a `.Spec.cs` suffix
 * (e.g. under Lib/Testing/) were counted as production — fixed by
 * isTestInfraDir(). Before #1897-r1, the blanket `*.g.cs` pattern hid
 * tracked files regardless of whether they were truly generated — fixed
 * by isGeneratedFile() and GENERATED_FILE_PATHS.
 */
...
/**
 * Whether a file is a spec/test/generated file. Three complementary
 * checks (issues #1821, #1895, #1897):
 *   - SPEC_PATTERNS: by name suffix (`.Spec.cs`, `.test.ts`, etc.)
