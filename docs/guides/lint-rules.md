...
- **Shipped in:** #1589 (warning level; invisible to `pnpm lint --quiet` until the mechanism lands). #1592 owns and tracks migration progress toward full `staticData.preload` coverage, but is not itself a live count — the current offender count comes from running the already-configured live `publy/route-query-preload` rule without `--quiet`. This guide does not pin a number, a ratchet, a parser, a baseline file, or a guard.
