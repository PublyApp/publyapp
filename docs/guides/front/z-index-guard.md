...
raw `z-index:` inside it reds), including deterministic `String.raw` tagged-template payloads, and CSS files imported with `?inline` (their authored file is
...
whose substitutions are static, a deterministic `String.raw` tagged template, or a `+` of two static
operands is a static payload at any depth,
