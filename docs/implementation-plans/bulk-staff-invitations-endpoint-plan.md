...
Each request item (email + profileIds array) creates **multiple** `Invitation` entities in the database—one per profileId.

For example:
- Request with 2 items, each having 2 profiles = 4 total `Invitation` entities created
- The `Created` count in the response reflects the total number of database entities, NOT the number of request items

This is because the `Invitation` entity has a unique token per record and represents a single email-profile-token combination.
...
| **Entity creation** | Individual `AddAsync` in loop | `AddRange` with batched INSERT | **50-70% faster** |
...
- ✅ **Sync `AddRange`** for bulk entity tracking (no false async overhead - see section 4.2)
...
**Note:** This plan uses synchronous methods (`AddRange`) for in-memory operations and async methods (`SaveChangesAsync`) for I/O. This differs from Node.js patterns - see section 4.2 for detailed explanation.
...
- `BulkStaffInvitationsCreated` with:
  - `int Created` – **total number of `Invitation` entities created** (NOT the number of request items).

Success response example (2 emails with 2 profiles each):
```json
{
  "created": 4
}
...
**Note:** The `Created` count is the sum of all profileIds across all request items, since each email×profile combination creates one `Invitation` entity with its own unique token.
...
- Ensure the array length is:
  ...
  - `<= MAX_BULK_SIZE` to prevent abuse.
    - **Define `MAX_BULK_SIZE` constant:**
      ...
      - Recommended value: `100` (representing 100 request items, which could expand to many more invitation entities).
...
- This prevents creating multiple invitation sets for the same email and provides clear feedback.
...
Call new batch validation service method (see section 4.1a):
```csharp
...
After all validation passes, call the new service method:
...
- Use `auditLogService.LogAsync` to log an aggregate action:
  ...
  - `details = new { Count = created, Scope = "Staff" }`
    - **Note:** Use `created` (the total entity count) rather than `invitations.Count` (the request item count).
...
Where `created` is the value returned from the service method (total number of `Invitation` entities created).
...
**Note:** The method returns `int` (total entities created) rather than a DTO, keeping the service layer focused on data operations.
...
2. **Create invitation entities (optimized for batch insertion):**
   ```csharp
   var invitationEntities = new List<Invitation>();
   var expiresAt = DateTime.UtcNow.AddDays(7);

   foreach (var item in invitations) {
       foreach (var profileId in item.ProfileIds) {
           // Generate unique token for each invitation entity
           var token = CryptoUtils.RandomString(_appSettings.Value.INVITATION_TOKEN_LENGTH);

           // Use factory to create invitation (handles email normalization)
           ...
           // Add to collection (not DbContext yet)
           invitationEntities.Add(invitation);
       }
   }

   // Add all entities at once using AddRange (more efficient than individual Add calls)
   _dbContext.Invitation.AddRange(invitationEntities);
   ```

   **Performance optimization notes:**
   - **Use synchronous `AddRange` (NOT `AddRangeAsync`):**
     - `AddRange` does **zero I/O** - it only tracks entities in memory
     - `AddRangeAsync` exists ONLY for custom value generators that need async I/O
     - We use Guid primary keys (no DB hit) and manual token generation (crypto random, in-memory)
     - Using `AddRangeAsync` here adds unnecessary async overhead (~5-10% slower) with no benefit
     - **This differs from Node.js:** .NET has a thread pool, so in-memory CPU work doesn't block other requests
   - `AddRange` is more efficient than multiple `Add` calls as it batches the tracking operations
   - Calculate `expiresAt` once outside the loop (same expiration for all invitations in batch)
   - Build entities in memory first, then add in bulk
...
3. **Save all changes** (single database INSERT with multiple rows):
   ...
   **Performance note:** EF Core batches multiple INSERTs into a single database command when possible, significantly improving performance for large batches.
...
5. **Log and return:**
   ```csharp
   var totalCreated = invitationEntities.Count;
...
- The `Invitation.CreateStaffInvitation` factory already normalizes emails to lowercase.
- The handler validation methods (`UserExistsAsync`, `PendingInvitationExistsAsync`) also normalize internally.

**Token Generation:**
Each `Invitation` entity requires a **unique token** due to the database unique constraint. The optimized implementation above generates a new cryptographically random token for each email×profile combination inside the nested loop, ensuring uniqueness.
...
- **Happy path**:
  ...
  - POST `/staff/invitations/bulk` with 2 items, each having 2 profile IDs.
  - Expect 200 with `{ "created": 4 }` (2 emails × 2 profiles = 4 invitation entities).
  - Verify DB has 4 invitation records with unique tokens for each email/profile combination.
...
### Data Model
- Each request item (email + profileIds) expands into **multiple** `Invitation` entities (one per profile)
- Each `Invitation` entity gets a **unique token** (required by database unique constraint)
- `Created` count represents total entities, not request items
...
- **Optimized entity creation:**
  - `AddRange` instead of individual `Add` calls (batches tracking operations)
  - Synchronous `AddRange` (no false async overhead)
  - `expiresAt` calculated once per batch instead of per entity
  - Entities built in memory first, then added in bulk
- **Single database roundtrip:**
  - One `SaveChangesAsync` call for all entities
...
**Expected performance:** With these optimizations, creating 100 invitations with 200 total entities should take under 500ms on typical hardware, compared to 5-10 seconds with N+1 queries.
...
This plan provides complete implementation guidance while maintaining consistency with PublyApp's architecture and conventions.
