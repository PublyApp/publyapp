...
Original location: docs/guides/frontend-coding-standards.md
Archive reason: Retired apps/front guide retained because archived designs depend on its MUI-era implementation rules.
Superseded by: AGENTS.md and docs/guides/front-2/conventions.md.
...
- Only lift state if multiple siblings truly need the same state _and_ you can keep it stable/minimal.
...
function StaffUsersPage() {
	...
	return (
		<QueryDisplay
			...
			EmptySlot={() => <Typography>No members found</Typography>}
		>
...
<QueryDisplay
	...
	ErrorSlot={({ error }) => (
		<Alert severity="error">Failed to load permissions: {error.message}</Alert>
	)}
	EmptySlot={() => (
...
- Debounce free-text search URL updates (300ms default) using
  `useUrlBackedDebouncedSearch` from `#app/hooks/table/use-url-backed-debounced-search.ts`.
- Use `useTableRowSelection` for shared row-selection state and derived selection values.
- Use `SelectionLockedControl` for search/filter controls disabled during selection mode.
- Use `downloadCsvFile` / `downloadJsonFile` from `#app/lib/export/download.ts` instead of
  duplicating client-side download plumbing.
