...
Original location: docs/misc/tenant-module-smoke-test-checklist.md
Archive reason: Retired apps/front smoke checklist retained because an archived design depends on its historical acceptance criteria.
Superseded by: nothing; retained as decision history.
...
- [x] A non-admin staff user with the specific tenant invitation revoke permission can revoke a pending tenant invitation successfully.
...
- [x] Suspending a globally active tenant user succeeds and keeps the row visible in the tenant users list as `Suspended`.
- [x] Reactivating a suspended tenant user succeeds and returns the row to `Active`.
- [x] Suspending a tenant user changes only the tenant membership state and does not globally suspend the underlying user identity.
- [x] Reactivating a tenant user changes only the tenant membership state and does not implicitly change unrelated memberships in other tenants.
- [x] The status control follows the single-choice selected-item pattern used elsewhere: current status selected, not disabled, no misleading pre-focus on the alternative option.
- [x] The status control shows `Active` and `Suspended` only; it does not offer `Pending`.
- [x] The last-active-admin invariant is enforced: suspending the last active tenant admin is rejected with clear error feedback.
- [x] Suspending a non-last tenant admin succeeds when at least one other active admin remains in the tenant.
- [x] A globally suspended user with a locally active membership remains visible in the tenant users list as `Globally suspended`.
- [x] A globally suspended user with a locally suspended membership also remains visible as `Globally suspended`.
- [x] If a user is globally suspended outside the tenant-users flow, any existing tenant-users row remains visible after refresh or query invalidation, but its effective status changes to `Globally suspended`.
- [x] The status chip for globally suspended rows is visually distinct from membership-only `Suspended`.
...
- [x] Filtering by `suspended` returns only suspended tenant users.
- [x] Filtering by `globally_suspended` returns only globally suspended tenant users.
...
- [x] Filtering by multiple comma-separated statuses (e.g., `active,suspended`) returns users matching any of the specified statuses.
- [x] Globally suspended users are excluded from `active`, `suspended`, and `active,suspended` filter results.
- [x] Changing search or filter values resets pagination correctly.
