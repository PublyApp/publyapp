...
- Env (`apps/front/src/lib/env.ts`): `VITE_ASP_SERVER_URL` (API base), `VITE_POSTHOG_PROJECT_TOKEN`.
...
- [ ] **Step 2: Turbo** — add `.output/**` to `build.outputs`; add `SERVER_API_BASE_URL` + `PUBLIC_API_BASE_URL` (+ `VITE_POSTHOG_PROJECT_TOKEN`) to `globalEnv`. `pnpm turbo build --filter=front-2-spike` caches `.output`.
