...
**Rev 3 changelog (plan round-2):** all 8 round-1 blocking closed; M2.0 URL params include `size`; M1.5a commits the Dockerfile before pushing the `:<sha>` image; M0.7 e2e support stack expanded.

**Rev 4 changelog (plan round-3):** corrected M0.7 — `request-counter` IS required (verified: `log-leak.spec.ts:19` scans its logs as a sink, Traefik routes the API through `request-counter:9443`, `SERVER_API_BASE_URL` points at it). The full e2e support stack is now `toxiproxy` + `request-counter` + `traefik-dynamic.test.yml`.
...
**Files:** Create `apps/front-2/playwright.config.ts`, `apps/front-2/docker-compose.test.yml`, `apps/front-2/traefik-dynamic.test.yml`, `apps/front-2/deploy/toxiproxy/toxiproxy.json`, `apps/front-2/deploy/request-counter/*`, `apps/front-2/e2e/helpers/*`. Reference (copy + path-fix): the same-named spike files + `apps/front-2-spike/deploy/{toxiproxy,request-counter}/`.
- [ ] **Step 1: Copy the FULL support stack the harvested specs depend on.** The spike `log-leak.spec.ts` topology genuinely requires all three sidecars — verified: `e2e/log-leak.spec.ts:19` scans `LOG_SINK_SERVICES = ['api','request-counter','front-2']`; `traefik-dynamic.test.yml:30` routes the API (TCP `api.front-2.localhost`) through `request-counter:9443`; and `docker-compose.test.yml` sets `SERVER_API_BASE_URL: http://request-counter:8800` → `request-counter` → `toxiproxy:8666` → API. So copy/adapt **`traefik-dynamic.test.yml` + `deploy/toxiproxy/toxiproxy.json` + `deploy/request-counter/*`** into `apps/front-2/`, fixing every spike path/hostname to front-2. (`request-counter` is the API proxy + log sink the log-leak sentinel reads; it is required, not optional.)
- [ ] **Step 2:** Create `apps/front-2/docker-compose.test.yml` (front-2 image + seeded Postgres + migrate + API + toxiproxy + request-counter + Traefik), `SERVER_API_BASE_URL: http://request-counter:8800`, **front-2 paths/URLs** only.
...
- [ ] **Step 5:** A trivial smoke spec + the harvested `log-leak.spec.ts` (toxiproxy fault + request-counter log-sink scan) both pass against the local stack — proving all three sidecars are wired. Commit.
