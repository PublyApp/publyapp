...
Add a repo-root `global.json` with `version: 10.0.300`, `rollForward: latestFeature`. This is issue
approach #2, justified here because the drift guard must build with a known SDK to be reliably green.
`latestFeature` keeps local devs on newer SDKs unblocked, while CI uses `setup-dotnet` honoring
`global.json` to install exactly `10.0.300` — the band that produced the committed spec. If a future
SDK feature band changes the generated output, the guard fails loudly and we regenerate + re-pin
