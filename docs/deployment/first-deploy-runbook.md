...
`VITE_POSTHOG_API_KEY` (analytics), `DI_MANIFEST_ENABLED` (false), `AUDIT_LOG_EXPORT_MAX_ROWS`
...
- **`... : not found` on image pull** → the images for that `RELEASE_TAG` were never published.
  ...
  the images must be built and pushed manually; mirror `deploy-images.yml` exactly and tag
  all three images with the **same full commit SHA**.
