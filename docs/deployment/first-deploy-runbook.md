...
| `SOCIAL_ACCOUNTS_MASTER_KEY` | REQUIRED secret — generate once with `openssl rand -base64 32` (32 bytes, base64). Protects the Data Protection key ring, which protects every `social_accounts.protected_credentials` blob; a missing or wrong-value key crash-loops `api`, `worker`, AND `migrate` at startup (the master-key witness refuses to boot). The committed all-zero `AAAA…AAA=` value in the repo is the documented BUILD/e2e placeholder only (Dockerfile doc-gen stages, `.github/workflows/ci.yml`, justfile) — never deploy it. Avoid `#` in the value (§8). |
...
`JOB_LEASE_SECONDS` (300), `JOB_QUEUE_DRAIN_BUDGET_SECONDS` (60),
`JOB_QUEUE_DRAIN_STALL_SECONDS` (120 — how long a due job may sit unclaimed before the api
reports it on the non-routing `/health/drain` surface; it never affects routing),
