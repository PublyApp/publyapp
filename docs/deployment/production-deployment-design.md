...
> normal one-shot migrate service plus API readiness and worker startup gates. See the
...
| 7 | Frontend target | **`apps/front-2`** (SSR), deployed under the generic service name `publyapp-front` to absorb the future front/front-2 rename. `apps/front` is retired and is not built for release. |
...
- **One-shot service (3):** Docker **Swarm ignores `depends_on`**, so the immutable migrator runs concurrently as a normal service that must run exactly once. API readiness and the worker startup gate block application work; a five-minute healthcheck startup grace covers the bounded wait. Because the instance turned out to be plain Compose, the migrate service carries **both** `restart: "no"` (Compose) and `deploy.restart_policy.condition: none` (Swarm), so it is correct under either runtime, and the ordering still comes from application readiness rather than `depends_on`.
...
|---------|-------|-----------|--------------|------------------|
| **publyapp-migrate** (EF bundle, one-shot service) | `migrate` | `api` | process exit code (non-zero remains failed) | — |
...
Deploy flow:
1. CI builds and pushes the three immutable-tagged images (`…:<release>`), including the migrator-bundle image.
...
- **Full `AppEnvironment` variable set** must be supplied to `api`, `worker`, and `migrate` — configuration validation is monolithic (`AppEnvironment.Initialize()`), so a missing var fails the process fast. `dokploy.yml` now declares the full set (it once declared only two). Secrets live in Dokploy's env management, never committed.
...
- **Connection pool caps** per role so `api` + `worker` + scheduler-leadership + LISTEN/NOTIFY connections stay under Postgres `max_connections` with an admin reserve. In force: **api `MaxPoolSize=50`, worker `MaxPoolSize=30`** against default `max_connections=100`; tune under load.
...
- Full env set declared (was 2 vars → instant fail-fast).
