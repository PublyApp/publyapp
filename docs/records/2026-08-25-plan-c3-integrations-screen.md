...
- Vertical slice stays under `apps/api/Modules/SocialAccounts` on the C2 branch — **the only backend code this plan writes is Task 2's Auth touch** (`GetUserAuthData.cs` + `GetUserAuthData.Spec.cs`); any SocialAccounts contract gap is recorded as an assumption, never invented here.
...
The private helpers Task 2 relies on are copied VERBATIM from `TenantPermissionFilter.Spec.cs` into this new spec (they are private there, so the new spec cannot reuse them) — appended inside `GetUserAuthDataSpec` exactly as they exist on develop (`CreateTenantProfileWithPermissionsAsync(Guid tenantId, IEnumerable<string> permissionKeys)` creating a `Profile.CreateTenantProfile` + one `ProfilePermission` row per key via the fixture's `AppDbContext`, and `AssignProfileToTenantUserAsync(string email, Guid tenantId, IReadOnlyCollection<Guid> profileIds)` resolving the tenant `UserAccount` by email and inserting `UserAccountProfile` links), plus their shared `CleanupTenantProfileArtifactsAsync` used by every non-admin fact's `finally`:
...
```csharp
	private async Task<Guid> CreateTenantProfileWithPermissionsAsync(
		...
		var profile = Profile.CreateTenantProfile(
			tenantId,
			"Get User Auth Data Spec " + Guid.NewGuid().ToString("N")[..8],
...
The page itself keeps its route shell (`WorkspacePageHeader titleKey="integrations"`, crumbs, i18nNamespaces — unchanged), swaps the three placeholder cards for one `DataTable` over `toSocialAccountRows(useSocialAccountsQuery(...).data)` with columns handle / provider (`t('provider-bluesky')`) / status pill / last success (`formatDateTime(row.lastSuccessAt, locale)` or `t('last-success-never')`) / visible-in / actions (`Reconnect`, `Disconnect` when `useCanManageSocialAccounts()`; Connect button in the header action slot likewise gated). Loading/empty/error flow through `DataTableQueryState` (`isPending`/`isError`/`onRetry`/emptyContent reusing `StateSurface`). The two test ids the RED test asserts are declared EXACTLY here so RED/GREEN cannot desync:

```tsx
// The DataTable's testId prop — DataTable renders it as its own shell
// data-testid (data-table.tsx:395):
<DataTable testId="tenant-settings-social-accounts-table" … />

// The empty state, rendered through DataTable's emptyContent → StateSurface,
// which forwards testId verbatim (state-surface.tsx):
emptyContent={(
	<StateSurface
...
(The page-level wrapper keeps the existing `data-testid="tenant-settings-integrations-page"`.)
