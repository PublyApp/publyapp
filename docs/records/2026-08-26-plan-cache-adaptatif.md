...
- Paquets ajoutés à `Directory.Packages.props` : `ZiggyCreatures.FusionCache` 2.7.1 et `ZiggyCreatures.FusionCache.Serialization.SystemTextJson` 2.7.1 (le sérialiseur System.Text.Json vit dans un paquet séparé ; identité exacte à confirmer au premier `dotnet restore`, qui échoue bruyamment si l'identité ou la version diffèrent — le compilateur ferme ce point, aucun risque silencieux).
...
- Citations develop : `.dump/citations-r1.md` (tip `198a6e4b7`, 14/14 PASS).
- FusionCache 2.7.1 : releases GitHub (v2.7.1 « Latest », 2026-08-20) ; `docs/MicrosoftHybridCache.md` (`AsHybridCache`, `AsKeyedHybridCache`, configuration au démarrage uniquement) ; licence MIT.
