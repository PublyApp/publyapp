...
The marketing pages (pricing, legal trio, about/contact/security, blog, changelog, plus the second-batch surfaces) were shipped on the retired old front — PR #362 (+ #339 homepage, siblings #664–#667, second batch #668–#673) — all closed 2026-07-21 when `apps/old-front` was retired (tag `old-front-final`). Everything in #368–#375 assumes those pages exist on the current front. They do not: `apps/front` has the landing page at `/`, the #1038 marketing shell whose nav/footer destinations are deliberately route-less until their routes exist, cookie consent (#1038), and a two-flag registry.

---

## Sources read while writing this plan

Every load-bearing claim below was verified against this tree at `develop` = `e13ee04a7` (this lane branches from it, 0/0 divergence):

| Claim | Source |
|---|---|
| What the five-phase PR delivered, phase by phase, with file paths | Closed PR [#362](https://github.com/PublyApp/publyapp/pull/362) body (read in full; file list confirms only 22 residual files touched on the pre-rename front) |
| Homepage rebuild + marketing-surface conventions codification | Closed PR [#339](https://github.com/PublyApp/publyapp/pull/339) body |
| Second-batch surfaces (comparison, feature pages, roadmap, customer story, SEO infra, cookie consent) and their locked scope decisions | Closed PRs [#671](https://github.com/PublyApp/publyapp/pull/671), [#669](https://github.com/PublyApp/publyapp/pull/669), [#670](https://github.com/PublyApp/publyapp/pull/670), [#668](https://github.com/PublyApp/publyapp/pull/668), [#672](https://github.com/PublyApp/publyapp/pull/672), [#673](https://github.com/PublyApp/publyapp/pull/673) bodies |
| The 2026-04-30…05-06 specs/plans survive in history | commit `a33270803` added `docs/superpowers/{specs,plans}/2026-05-0{1,2,3,6}-marketing-*` (12 files); retrievable via `git show a33270803:<path>` |
...
| Marketing/auth SSR; authed CSR; `createServerFn` boundary rules | `docs/guides/front/conventions.md` "Rendering Strategy", "Server-Function Boundary" |
...
| `/pricing` | #362 P1 | Yes: tiers (creator 19/15, scale 49/39, enterprise custom), 6-category comparison matrix, 5 FAQs (retirement record §_data/pricing) | Rebuild page; **prices ship struck-through + beta note** (owner rule, see D7) |
| `/terms` `/privacy` `/cookies` | #362 P2 | Yes: TOC ids, section copy, lastUpdated trio (retirement record) | Rebuild via one `LegalDocPage`-equivalent shell + data modules; `/cookies` wires the **existing** prefs drawer |
| `/about` | #362 P3 | Copy yes; 12 team members are fictional placeholders (#368 A3) | Rebuild; team grid uses **initials avatars**, never stock portraits |
| `/contact` | #362 P3 | Yes: channels, support tiers, topics, 4 FAQs, Zod schema preserved verbatim in retirement record | Rebuild form (RHF+Zod via front field wrappers); **honest placeholder submit** (D8); backend = #369 B1 |
| `/security` | #362 P3 | Structure yes; badges/sub-processors are unverified claims (#368 A4) | Rebuild; claims carry visible "pending review" treatment; `noindex` question deferred to #374 |
| `/blog` + `/blog/:slug` | #362 P4 | Data shapes yes; 13 posts (3 unpublished)/4 tags are placeholders (#368 A1) | Phase 2 rebuild; static typed data modules; JSX-as-content bodies |
| `/changelog` + `/changelog/:year` | #362 P5 | Shapes yes; 40 entries 2015–2026 are placeholders (#368 A2) | Phase 2 rebuild; `/changelog` redirects to latest year |
| marketing catch-all 404 | #362 P3 | n/a | **Exists**: root `notFoundComponent` → `View404` inside `MarketingLayout` chrome. Polish optional, lowest priority |
...
Semantics carried over from the retired registry: **one flag drives route registration AND nav/footer visibility** — `marketing-nav.ts` attaches `to` only when the flag is on (its closed `MarketingRoutePath` union extends per route; `marketing-nav.test.ts` keeps every destination honest against the real route tree; "no dead ends" rule intact). Enforcement of the owner rule in production is structural: front flags are build-time frozen and **no Dockerfile ARG/ENV pair exists for marketing flags**, so nothing ships enabled unless someone deliberately adds the ARG and builds an image with it — and #368's content gates must close first. Prices and claims inside enabled-for-preview pages carry their own honest treatment (D7/D8) so even a preview build doesn't present fabrication as fact.
