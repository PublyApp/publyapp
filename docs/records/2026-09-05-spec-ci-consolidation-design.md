...
| API path coverage guard is reachable on its own inputs | `verification` | Unconditional invocation; ordinary path-filter behavior is tested against the real barrier workflows and representative fixtures |
