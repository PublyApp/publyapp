...
[x] **Gates under `heavy.sh`:** full Publishing + Posts + Invitations(spec sanity) suites; `just build-api`; `just ci-front`; `just ci-migration-expand-contract`; `just knip` (execution: API Publishing|Posts|Invitations suites 186/186, build-api OK, ci-front OK, migration OK; knip already fails on develop — 87 files, exit 1 verified on main checkout; after fix, no file from this lane's diff appears in the report).
